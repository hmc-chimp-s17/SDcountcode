...
# Fixed LHS edges (always present)
FIXED = [
...
def check_three_paths(edges):
    """
    Check if graph forms 3 disjoint paths: s1→t1, s2→t2, s3→t3.
    ...
    # Check each component is a valid path
    for comp in components:
        s_in_comp = [v for v in comp if v in S]
        t_in_comp = [v for v in comp if v in T]

        # Must have exactly one s and one t with matching index
        if len(s_in_comp) != 1 or len(t_in_comp) != 1:
            return False
        if s_in_comp[0][1] != t_in_comp[0][1]:
            return False
...
if __name__ == "__main__":
    print("="*60)
    print("6j Symbol Configuration Counter (Simplified)")
    print("="*60)

    by_cycles, t_same_type = count_all()
    ...
    cycle_dir = "cycle_examples"
    t_dir = "t_same_type_all"
    os.makedirs(cycle_dir, exist_ok=True)
    os.makedirs(t_dir, exist_ok=True)

    # Draw 1-cycle examples (special: all t's connect to same type)
    t_same_1cycle = [(rhs, combined, cycles, nc) for rhs, combined, cycles, nc in t_same_type if nc == 1]
    if t_same_1cycle:
        print(f"\nDrawing {len(t_same_1cycle)} examples with 1 cycle (all t's → same type)...")
        for i, (rhs, combined, cycles, nc) in enumerate(t_same_1cycle):
            t_type = all_t_same_type(rhs)
            title = f"1 Cycle (all t→{t_type}) - Example {i+1}/{len(t_same_1cycle)}"
            draw(combined, cycles, title, f"{cycle_dir}/1cycle_{i+1}.png")

    # Draw 2-cycle examples
    if 2 in by_cycles:
        print(f"\nDrawing {len(by_cycles[2])} examples with 2 cycles...")
        for i, (rhs, combined, cycles) in enumerate(by_cycles[2]):
            title = f"2 Cycles - Example {i+1}/{len(by_cycles[2])}"
            draw(combined, cycles, title, f"{cycle_dir}/2cycles_{i+1}.png")

    # Draw 3-cycle examples
    if 3 in by_cycles:
        print(f"\nDrawing {len(by_cycles[3])} example(s) with 3 cycles...")
        for i, (rhs, combined, cycles) in enumerate(by_cycles[3]):
            title = f"3 Cycles - Example {i+1}/{len(by_cycles[3])}"
    ...
    for i, (rhs, combined, cycles, nc) in enumerate(t_same_type):
        t_type = all_t_same_type(rhs)
        title = f"All t→{t_type}, {nc} cycles - #{i+1}/{len(t_same_type)}"
