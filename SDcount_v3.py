...
def count_all(fixed_edges, label):
    ...
    print(f"\n{'='*60}")
    print(f"Analyzing {label} configuration")
    print(f"{'='*60}")
...
def print_results(label, by_cycles, t_same_type):
    ...
    print("\n" + "="*60)
    print(f"{label} - TOTAL VALID: {total}")
    print("="*60)
...
def draw_results(label, by_cycles, t_same_type, base_dir):
    ...
    cycle_dir = f"{base_dir}_cycle_examples"
    t_dir = f"{base_dir}_t_same_type_all"
    ...
    if t_same_with_cycles:
        ...
        for i, (rhs, combined, cycles, nc) in enumerate(t_same_with_cycles):
            ...
            title = f"{label} - {nc} Cycle(s) (all t→{t_type}) - Example {i+1}/{len(t_same_with_cycles)}"
    ...
    if 2 in by_cycles:
        ...
        for i, (rhs, combined, cycles) in enumerate(by_cycles[2]):
            title = f"{label} - 2 Cycles - Example {i+1}/{len(by_cycles[2])}"
    ...
    if 3 in by_cycles:
        ...
        for i, (rhs, combined, cycles) in enumerate(by_cycles[3]):
            title = f"{label} - 3 Cycles - Example {i+1}/{len(by_cycles[3])}"
    ...
    for i, (rhs, combined, cycles, nc) in enumerate(t_same_type):
        t_type = all_t_same_type(rhs)
        title = f"{label} - All t→{t_type}, {nc} cycles - #{i+1}/{len(t_same_type)}"
...
if __name__ == "__main__":
    print("="*60)
    print("6j Symbol Configuration Counter - Two Configurations")
    print("="*60)
