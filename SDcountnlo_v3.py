"""
SDcountnlo_v3.py - Check ALL Three Paths

Find matchings where ANY of the three paths (s1-t1, s2-t3, s3-t2) has at least one cycle
after merging a,b,c vertices. Self-loops are NOT counted as cycles.
...
5. Count cycles in each merged path (excluding self-loops)
6. Keep only matchings where ANY path has >=1 cycles
"""
...
def find_matchings_with_cycles_in_any_path(fixed_edges, label):
    """Find all matchings where ANY path has >=1 cycles after merging."""
    print(f"\n{'='*60}")
    ...
    # Check which have >=1 cycles in ANY path after merging (excluding self-loops)
    results = []

    print(f"\nChecking all three paths after merging...")
    ...
    for idx, rhs, combined, after_removal, cycles_removed in one_cycle_nonspecial:
        ...
        # Merge and count cycles for each path
        merged_s1_t1 = merge_vertices(s1_t1_path)
        merged_s2_t3 = merge_vertices(s2_t3_path)
        merged_s3_t2 = merge_vertices(s3_t2_path)

        cycles_s1_t1 = count_cycles(merged_s1_t1)
        cycles_s2_t3 = count_cycles(merged_s2_t3)
        cycles_s3_t2 = count_cycles(merged_s3_t2)
    ...
    print(f"\nResults (ANY path with >=1 cycles after merging, excluding self-loops):")
...
def save_results(label, results, output_dir):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{label} - ANY Path with >=1 Cycles After Merging\n")
        f.write("="*70 + "\n\n")
...
def main():
    print("="*60)
    print("SDcountnlo_v3: ANY Path Cycle Analysis")
    print("="*60)

    # Process 6J
    results_6j = find_matchings_with_cycles_in_any_path(FIXED_6J, "6J")
    save_results("6J", results_6j, "nlo_v3_results/6J")

    # Process PILLOW
    results_pillow = find_matchings_with_cycles_in_any_path(FIXED_PILLOW, "PILLOW")
    save_results("PILLOW", results_pillow, "nlo_v3_results/PILLOW")
