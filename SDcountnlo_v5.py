"""
SDcountnlo_v5.py - s2-t3 Path with a-c Splitting
...
6. Count cycles in the s2-t3 path (excluding self-loops)
...
def extract_s2_t3_path(edges):
    """Extract the s2-t3 path from the graph."""
    adj = build_adj(edges)
...
def find_matchings_with_s2_t3_cycles(fixed_edges, label):
    """Find all matchings where s2-t3 path has >=1 cycles after merging and splitting a-c."""
    print(f"\n{'='*60}")
    ...
    # Check which have >=1 cycles in s2-t3 after merging and splitting (excluding self-loops)
    results = []

    print(f"\nChecking s2-t3 path after merging and splitting a-c edges...")
    ...
    print(f"\nResults (s2-t3 path with >=1 cycles after merging+splitting, excluding self-loops):")
...
def save_results(label, results, output_dir):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{label} - s2-t3 Path with >=1 Cycles After Merging+Splitting a-c\n")
        f.write("="*70 + "\n\n")
...
def main():
    print("="*60)
    print("SDcountnlo_v5: s2-t3 Path Cycle Analysis (with a-c splitting)")
    print("="*60)

    # Process 6J
    results_6j = find_matchings_with_s2_t3_cycles(FIXED_6J, "6J")
    save_results("6J", results_6j, "nlo_v5_results/6J")

    # Process PILLOW
    results_pillow = find_matchings_with_s2_t3_cycles(FIXED_PILLOW, "PILLOW")
    save_results("PILLOW", results_pillow, "nlo_v5_results/PILLOW")
