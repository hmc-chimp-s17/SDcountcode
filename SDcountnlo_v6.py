"""
SDcountnlo_v6.py - s3-t2 Path with a-c Splitting

Find matchings where the s3-t2 path has at least one cycle after merging a,b,c vertices
AND splitting a-c edges into a-b and b-c. Self-loops are NOT counted as cycles.

Steps:
1. Find non-special 1-cycle and special 0-cycle matchings with NLO pairing (s1-t1, s2-t3, s3-t2)
2. Remove cycles
3. Extract the s3-t2 path
4. Merge vertices: a1,a2,a3 → a; b1,b2,b3 → b; c1,c2,c3 → c
5. Split a-c edges: each a-c edge becomes a-b and b-c
6. Count cycles in the s3-t2 path (excluding self-loops)
7. Keep only matchings with >=1 cycles
"""
...
def extract_s3_t2_path(edges):
    """Extract the s3-t2 path from the graph."""
    adj = build_adj(edges)
...
def find_matchings_with_s3_t2_cycles(fixed_edges, label):
    """Find all matchings where s3-t2 path has >=1 cycles after merging and splitting a-c."""
    print(f"\n{'='*60}")
    ...
    # Check which have >=1 cycles in s3-t2 after merging and splitting (excluding self-loops)
    results = []

    print(f"\nChecking s3-t2 path after merging and splitting a-c edges...")

    for idx, rhs, combined, after_removal, cycles_removed in one_cycle_nonspecial:
        s3_t2_path = extract_s3_t2_path(after_removal)
        merged_path = merge_vertices(s3_t2_path)
        split_path = split_ac_edges(merged_path)
        num_cycles_in_path = count_cycles(split_path)
    ...
    print(f"\nResults (s3-t2 path with >=1 cycles after merging+splitting, excluding self-loops):")
...
def save_results(label, results, output_dir):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{label} - s3-t2 Path with >=1 Cycles After Merging+Splitting a-c\n")
        f.write("="*70 + "\n\n")
        ...
        for i, r in enumerate(results, 1):
            ...
            f.write(f"  Cycles in s3-t2: {r['num_cycles']}\n")
            f.write(f"  RHS edges: ")
...
def main():
    print("="*60)
    print("SDcountnlo_v6: s3-t2 Path Cycle Analysis (with a-c splitting)")
    print("="*60)

    # Process 6J
    results_6j = find_matchings_with_s3_t2_cycles(FIXED_6J, "6J")
    save_results("6J", results_6j, "nlo_v6_results/6J")

    # Process PILLOW
    results_pillow = find_matchings_with_s3_t2_cycles(FIXED_PILLOW, "PILLOW")
    save_results("PILLOW", results_pillow, "nlo_v6_results/PILLOW")
