"""
SDcountnlo_v7.py - s1-t1 Path with a-c Splitting

Find matchings where the s1-t1 path has at least one cycle after merging a,b,c vertices
...
def extract_s1_t1_path(edges):
    """Extract the s1-t1 path from the graph."""
    adj = build_adj(edges)
...
def find_matchings_with_s1_t1_cycles(fixed_edges, label):
    """Find all matchings where s1-t1 path has >=1 cycles after merging and splitting a-c."""
    print(f"\n{'='*60}")
    ...
    # Check which have >=1 cycles in s1-t1 after merging and splitting (excluding self-loops)
    results = []

    print(f"\nChecking s1-t1 path after merging and splitting a-c edges...")

    for idx, rhs, combined, after_removal, cycles_removed in one_cycle_nonspecial:
        s1_t1_path = extract_s1_t1_path(after_removal)
        merged_path = merge_vertices(s1_t1_path)
    ...
    print(f"\nResults (s1-t1 path with >=1 cycles after merging+splitting, excluding self-loops):")
...
def save_results(label, results, output_dir):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{label} - s1-t1 Path with >=1 Cycles After Merging+Splitting a-c\n")
        f.write("="*70 + "\n\n")
        ...
        for i, r in enumerate(results, 1):
            ...
            f.write(f"  Cycles in s1-t1: {r['num_cycles']}\n")
            f.write(f"  RHS edges: ")
...
def main():
    print("="*60)
    print("SDcountnlo_v7: s1-t1 Path Cycle Analysis (with a-c splitting)")
    print("="*60)

    # Process 6J
    results_6j = find_matchings_with_s1_t1_cycles(FIXED_6J, "6J")
    save_results("6J", results_6j, "nlo_v7_results/6J")

    # Process PILLOW
    results_pillow = find_matchings_with_s1_t1_cycles(FIXED_PILLOW, "PILLOW")
    save_results("PILLOW", results_pillow, "nlo_v7_results/PILLOW")
