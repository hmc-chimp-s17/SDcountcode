"""
SDcountnlo_v8.py - s1-t1 Path with Cycles, s2-t3 and s3-t2 with Zero Cycles

Find matchings where:
- s1-t1 path has >=1 cycles after merging a,b,c vertices AND splitting a-c edges
- s2-t3 path has 0 cycles after merging a,b,c vertices AND splitting a-c edges
- s3-t2 path has 0 cycles after merging a,b,c vertices AND splitting a-c edges
...
7. Keep only matchings where s1-t1 has >=1 cycles AND s2-t3 has 0 cycles AND s3-t2 has 0 cycles
"""
...
def find_matchings_with_s1_t1_only_cycles(fixed_edges, label):
    """Find matchings where s1-t1 has >=1 cycles AND s2-t3 has 0 cycles AND s3-t2 has 0 cycles."""
    print(f"\n{'='*60}")
    ...
    # Check which have >=1 cycles in s1-t1 AND 0 cycles in s2-t3 AND 0 cycles in s3-t2
    results = []

    print(f"\nChecking s1-t1 with >=1 cycles AND s2-t3 with 0 cycles AND s3-t2 with 0 cycles...")
    ...
    for idx, rhs, combined, after_removal, cycles_removed in one_cycle_nonspecial:
        ...
        # Check if s1-t1 has >=1 cycles AND s2-t3 has 0 cycles AND s3-t2 has 0 cycles
        if cycles_s1_t1 >= 1 and cycles_s2_t3 == 0 and cycles_s3_t2 == 0:
            results.append({
    ...
    print(f"\nResults (s1-t1 with >=1 cycles, s2-t3 with 0 cycles, s3-t2 with 0 cycles):")
...
def save_results(label, results, output_dir):
    ...
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{label} - s1-t1 with >=1 Cycles, s2-t3 and s3-t2 with 0 Cycles\n")
        f.write("="*70 + "\n\n")
...
def main():
    print("="*60)
    print("SDcountnlo_v8: s1-t1 Only Cycle Analysis")
    print("(s1-t1 with >=1 cycles, s2-t3 and s3-t2 with 0 cycles)")
    print("="*60)

    # Process 6J
    results_6j = find_matchings_with_s1_t1_only_cycles(FIXED_6J, "6J")
    save_results("6J", results_6j, "nlo_v8_results/6J")

    # Process PILLOW
    results_pillow = find_matchings_with_s1_t1_only_cycles(FIXED_PILLOW, "PILLOW")
    save_results("PILLOW", results_pillow, "nlo_v8_results/PILLOW")
