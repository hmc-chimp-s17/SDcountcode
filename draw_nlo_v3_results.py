"""
Draw results from SDcountnlo_v3.py showing all three paths.

LEFT: Complete graph with removed cycles highlighted in red, all three paths color-coded
RIGHT: All three merged paths showing cycles in each
"""
...
from SDcountnlo_v3 import (
    check_nlo_paths, extract_path, merge_vertices, find_cycle, count_cycles
)
...
def draw_three_paths_comparison(result, label, filename):
    ...
    # ===== RIGHT: Only merged paths with cycles in 2D layout =====
    ...
    # Path info: name, merged_edges, color, s_node, t_node
    path_info_map = {
        's1_t1': (result['merged_s1_t1'], 'green', ('s', 1), ('t', 1)),
        's2_t3': (result['merged_s2_t3'], 'blue', ('s', 2), ('t', 3)),
        's3_t2': (result['merged_s3_t2'], 'purple', ('s', 3), ('t', 2)),
    }

    # Get cycles and draw only paths with cycles
    for path_name in paths_with_cycles:
        merged_edges, color, s_node, t_node = path_info_map[path_name]
        y_offset = path_y_map[path_name]

        # Get cycles for this path
        cycles = []
        remaining = merged_edges[:]
        while True:
        ...
        # Define 2D positions for this path section (like v2 - triangular layout)
    ...
    title_text = "Merged Paths with Cycles (cycles in red)\n"
    cycle_info = []
    if result['cycles_s1_t1'] > 0:
...
def draw_all_v3_results(label, results, output_dir):
    """Draw all matchings from v3 results."""
...
def main():
    print("="*60)
    print("Drawing all SDcountnlo_v3 results")
    print("="*60)

    # Load results from v3
    from SDcountnlo_v3 import find_matchings_with_cycles_in_any_path

    # Process 6J
    results_6j = find_matchings_with_cycles_in_any_path(FIXED_6J, "6J")
    draw_all_v3_results("6J", results_6j, "nlo_v3_results/6J/drawings")

    # Process PILLOW
    results_pillow = find_matchings_with_cycles_in_any_path(FIXED_PILLOW, "PILLOW")
    draw_all_v3_results("PILLOW", results_pillow, "nlo_v3_results/PILLOW/drawings")
