"""
Draw results from SDcountnlo_v5.py showing s2-t3 path with a-c splitting.

LEFT: Complete graph with removed cycles highlighted in red, s2-t3 path in blue
RIGHT: Merged+split s2-t3 path showing cycles in red
"""
...
from SDcountnlo_v5 import (
    check_nlo_paths, extract_s2_t3_path, merge_vertices, split_ac_edges, find_cycle, count_cycles
)


def draw_comparison(result, label, filename):
    """Draw side-by-side comparison: complete graph vs merged+split s2-t3 path."""
    ...
    # RIGHT: merged+split s2-t3 path with cycles
    # Get all cycles
    cycles = []
    ...
    ax2.axis('off')
    title_text = f"Merged+Split s2-t3 Path ({result['num_cycles']} cycles, cycles in red)"
...
def draw_all_v5_results(label, results, output_dir):
    """Draw all matchings from v5 results."""
...
def main():
    print("="*60)
    print("Drawing all SDcountnlo_v5 results")
    print("="*60)

    # Load results from v5
    from SDcountnlo_v5 import find_matchings_with_s2_t3_cycles

    # Process 6J
    results_6j = find_matchings_with_s2_t3_cycles(FIXED_6J, "6J")
    draw_all_v5_results("6J", results_6j, "nlo_v5_results/6J/drawings")

    # Process PILLOW
    results_pillow = find_matchings_with_s2_t3_cycles(FIXED_PILLOW, "PILLOW")
    draw_all_v5_results("PILLOW", results_pillow, "nlo_v5_results/PILLOW/drawings")
