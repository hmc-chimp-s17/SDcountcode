"""
Draw results from SDcountnlo_v6.py showing s3-t2 path with a-c splitting.

LEFT: Complete graph with removed cycles highlighted in red, s3-t2 path in purple
RIGHT: Merged+split s3-t2 path showing cycles in red
"""
...
from SDcountnlo_v6 import (
    check_nlo_paths, extract_s3_t2_path, merge_vertices, split_ac_edges, find_cycle, count_cycles
)


def draw_comparison(result, label, filename):
    """Draw side-by-side comparison: complete graph vs merged+split s3-t2 path."""
    ...
    vcolors = {
        **{v: 'lightblue' for v in [('s', 3)]},
        **{v: 'plum' for v in [('t', 2)]},
        ('a',): 'lightgreen', ('b',): 'lightyellow', ('c',): 'lightcoral',
    }

    # LEFT: Complete graph before cycle removal (highlight removed cycles and s3-t2 path)
    s3_t2_vertices = set([u for u, _ in result['s3_t2_path']] + [v for _, v in result['s3_t2_path']])
    ...
    for u, v in result['combined']:
        if u in pos_orig and v in pos_orig:
            ...
            # Color priority: removed cycles (red) > s3-t2 path (purple) > other (gray)
    ...
    for v in all_vertices:
        if v in pos_orig:
            ...
            # Highlight s3-t2 path vertices with thicker border
            ec_width = 3 if v in s3_t2_vertices else 2
    ...
    ax1.axis('off')
    title_text = f"Complete Graph ({result['cycles_removed']} cycle removed, s3-t2 in purple)"
    ax1.set_title(title_text, fontsize=12, fontweight='bold')

    # RIGHT: merged+split s3-t2 path with cycles
    # Get all cycles
    cycles = []
    ...
    ax2.axis('off')
    title_text = f"Merged+Split s3-t2 Path ({result['num_cycles']} cycles, cycles in red)"
...
def draw_all_v6_results(label, results, output_dir):
    """Draw all matchings from v6 results."""
...
def main():
    print("="*60)
    print("Drawing all SDcountnlo_v6 results")
    print("="*60)

    # Load results from v6
    from SDcountnlo_v6 import find_matchings_with_s3_t2_cycles

    # Process 6J
    results_6j = find_matchings_with_s3_t2_cycles(FIXED_6J, "6J")
    draw_all_v6_results("6J", results_6j, "nlo_v6_results/6J/drawings")

    # Process PILLOW
    results_pillow = find_matchings_with_s3_t2_cycles(FIXED_PILLOW, "PILLOW")
    draw_all_v6_results("PILLOW", results_pillow, "nlo_v6_results/PILLOW/drawings")
