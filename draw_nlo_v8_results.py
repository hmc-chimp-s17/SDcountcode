"""
Draw results from SDcountnlo_v8.py showing s1-t1 path exclusively with cycles.

LEFT: Complete graph with removed cycles highlighted in red, s1-t1 path in green
RIGHT: Merged+split s1-t1 path showing cycles in red (s2-t3 and s3-t2 have 0 cycles)
"""
...
def draw_comparison(result, label, filename):
    ...
    for u, v in result['split_s1_t1']:
        if u in pos_merged and v in pos_merged:
            ...
            if count > 1:
                ...
                offset_idx = 0
                for i, (u2, v2) in enumerate(result['split_s1_t1']):
    ...
    ax2.axis('off')
    title_text = f"s1-t1 Path Only ({result['cycles_s1_t1']} cycles)\ns2-t3: 0 cycles, s3-t2: 0 cycles"
...
def draw_all_v8_results(label, results, output_dir):
    """Draw all matchings from v8 results."""
...
def main():
    print("="*60)
    print("Drawing all SDcountnlo_v8 results")
    print("="*60)

    # Load results from v8
    from SDcountnlo_v8 import find_matchings_with_s1_t1_only_cycles

    # Process 6J
    results_6j = find_matchings_with_s1_t1_only_cycles(FIXED_6J, "6J")
    draw_all_v8_results("6J", results_6j, "nlo_v8_results/6J/drawings")

    # Process PILLOW
    results_pillow = find_matchings_with_s1_t1_only_cycles(FIXED_PILLOW, "PILLOW")
    draw_all_v8_results("PILLOW", results_pillow, "nlo_v8_results/PILLOW/drawings")
