"""
Draw all 1-cycle non-special matchings that pass the untwisted test.
...
def process_configuration(fixed_edges, config_label, output_dir):
    """Process a single configuration and draw all 1-cycle non-special matchings."""
    print("\n" + "="*60)
    ...
    # Find 1-cycle non-special matchings that pass untwisted test
    one_cycle_matchings = []
    ...
    for idx, rhs, combined in valid_matchings:
        ...
        if num_cycles == 1 and check_three_paths_structure(after):
            ...
            one_cycle_matchings.append((idx, rhs, combined, cycles))

    print(f"Found {len(one_cycle_matchings)} non-special 1-cycle matchings")

    # Draw all of them
    for i, (idx, rhs, combined, cycles) in enumerate(one_cycle_matchings, 1):
        title = f"{config_label} - 1 cycle (untwisted) - Matching {i}/{len(one_cycle_matchings)} (index {idx})"
        filename = f"{output_dir}/matching_{i:03d}_idx{idx}.png"
        draw_untwisted(combined, cycles, title, filename)

    # Save summary
    with open(f"{output_dir}/summary.txt", 'w') as f:
        f.write(f"{config_label} - All 1-cycle non-special matchings (untwisted)\n")
        f.write("="*60 + "\n\n")
        f.write(f"Total matchings: {len(one_cycle_matchings)}\n\n")
...
def main():
    print("="*60)
    print("Drawing all 1-cycle untwisted non-special matchings")
    print("="*60)

    # Create main output directory
    base_dir = "untwisted_1cycle_all"
